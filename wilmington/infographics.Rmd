...
Surplus food is _“All **food that goes unsold or unused by a business or that goes uneaten at home** – including food and inedible parts (e.g., peels, pits, bones) that are donated, fed to animals, repurposed to produce other products, composted, anaerobically digested, or wasted.”_ ([ReFED, Insights Engine Glossary, p.1](https://insights.refed.com/uploads/documents/refed-insights-engine-glossary-vfinal-2021.2.2.pdf?_cchid=7d0e044d61127907795816d134af4e67)).
...
# Surplus food: magnitude

While at first sight, this might seem neglibile at an individual level, aggregated figures for Delaware and nationwide tell us otherwise: **The costs of food waste are very high in terms of money and weight**


```{r food_waste_tonnes}
surplus_magnitude <- tibble(
  location = c("U.S.", "Delaware"),
  weight_year = c(80.8*10^6, 215000)
)

surplus_magnitude %>% 
  e_charts(location) %>% 
  e_pictorial(weight_year, symbol = ea_icons("user"), 
              symbolRepeat = TRUE, z = -1,
              symbolSize = c(20, 20)) %>% 
  e_theme("westeros") %>%
  e_title("Food waste in Tonnes/year") %>% 
  e_flip_coords() %>%
  # Hide Legend
  e_legend(show = FALSE) %>%
  # Remove Gridlines
...
2. **Recycled** such as through composting, feed for animals, anaerobic digestion, or industrial use
2. **Wasted** food is “Uneaten food and inedible parts that end up being landfilled, incinerated, disposed of down the sewer, dumped, spread onto land, or simply not harvested.” (ReFED, Insights Engine Glossary, p.2)


```{r spf_management}
spf_managemen <- tibble(
...
ggplot(spf_managemen, aes(area = value, fill = variable, label = label)) +
...
More than half of the food waste goes to Landfills.
A notable difference from Delaware is that food is the lack of incineration.
...
* Food waste in MSW: 63.1 million tons (21.6%)
  ...
  * Management pathway:
    ...
    * Incineration, landfill: 42.8 million tons -> I calculated landfill based on the fact that it was stated previously that it accounts to 24% and incinerator is the difference.
    * Other management methods: 17.7 million tons
    
```{r eval=FALSE, include=FALSE}
msw_distribution_us <- read_csv("data/msw_distribution_us.csv")

treemap <- plot_ly(
  type = "treemap",
  labels = msw_distribution_us$label,
  parents = msw_distribution_us$parent,
  values = msw_distribution_us$weight
)

treemap <- treemap %>% 
  layout(
    title = "MSW Distribution (US)"
  )
...
msw_distribution_us <- read_csv("data/msw_distribution_us.csv") %>% 
  mutate(label_long = paste0(label, "\n(", weight, " million tonnes)"))

ggplot(msw_distribution_us, aes(area = weight, fill = label, label = label_long)) +
  geom_treemap(show.legend  = FALSE) +
  geom_treemap_text(colour = "white", place = "left",
                grow = TRUE) +
  labs(
...
Carbon footprint: 270 million metric tons CO2eq (3.34 metric tons per ton wasted food)

Water footprint: 14.1 trillion gallons (174 gallons per ton wasted food)
