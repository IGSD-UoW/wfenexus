...
g <- ggplot(ingredients_used_df, 
  ...
  geom_point_interactive(aes(alpha = 0.5)) +
...
g <- girafe(
  ggobj = g, width_svg = 6, height_svg = 6,
  options = list(
...
# tmp_dishes_df <- dishes_df %>% 
#   select(name, type_label, water_used, rating_average) %>% 
#   group_by(name, type_label) %>% 
#   summarise(water_used = mean(water_used, na.rm = TRUE),
#             rating_average = mean(rating_average, na.rm = TRUE), 
#             total = n()) %>% 
#   filter(!is.na(water_used))
# 
# # Dishes scatterplot.
# gd <- ggplot(tmp_dishes_df, 
#              aes(x = rating_average, y = water_used, label = name,
#                  color = type_label)) +
#   geom_point() +
#   labs(title = "Meals' usage vs. water footprint",
#        subtitle = "This is a subtitle",
#        x = "Rating average",
#        y = "Total used water (m3)",
#        # color = "Type of food",
#        size = NULL,
#        alpha = NULL)
# 
...
g <- ggplotly(g, tooltip = c("weight_grams", "world_total", "name_en"))
...
subplot(g, gd, nrows=1, titleX = TRUE, titleY = TRUE)
