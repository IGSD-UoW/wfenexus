...
TODO: 
* add dish composition (it can be downloaded [from the tool](https://creatinginterfaces.demo.52north.org/slupsk-tool/kindergartendishs.json)!), ideally as nested component.
* Fix the average calculations
* Recode waste: order levels + change label
* Hide type column or move it under the name + rename factor + reorder

```{r}
df <- dishes_df %>% 
  relocate(picture) %>% 
  select(-id, -kindergarten_id, -date, -ends_with("label"))
